...
def calc_futureval(P: float, rate: float) -> list[float]:
    """Calculate the future value of an investment with principal P
    and interest rate RATE (as a quantity, not a percentage) until P
    reaches double its initial value.  Returns a list of numbers,
    representing the principal at the end of each of the compounding
    periods."""
    values: list[float] = [P] # Initial principal
    while P < 2 * values[0]:
        interest = P * rate
        P = P + interest
        values.append(P)
...
def main(args: list[str]) -> int:
    P, rate = read_parameters()
    # Echo back the parameters
    print('Investing $', P, 'at', rate, '% until the original investment doubles:')

    values = calc_futureval(P, rate/100)
    print_table(values)
    graph(values)
