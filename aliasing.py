...
def main(args: list[str]) -> int:
    ...
    x_label = Text(Point(-.5, -0.02 * Pmax), '')
    x_label.draw(w)
    y_label = Text(Point(-.5, P + 0.02 * Pmax), '')
    y_label.draw(w)
    ...
    for i in range(periods):
        ...
        # Data labels
        label: Text = x_label # Aliasing! Both x_label and label now refer
                              # to the same object, which *can* be changed
        label.move(i+0.5 - x_label.getAnchor().getX(), 0)
        label.setText(str(i+1))
        #Text(Point(i+0.5, -0.02 * Pmax), str(i+1))
        #label.draw(w)

        label = y_label.clone() # No aliasing.  label and y_label refer to
                                # *different* objects.
        label.move((i+0.5) - y_label.getAnchor().getX(), 
                   (P + 0.02*Pmax) - y_label.getAnchor().getY())
        label.setText('$' + str(round(P, 2)))
        # Text(Point(i+0.5, P + 0.02 * Pmax), '$' + str(round(P, 2)))
        label.draw(w)  # Clones are always created undrawn
